...
def test_comment_lifecycle(client: TestClient):
    ...
    b = client.post(
        "/boards", json={"name": "CBoard", "category": "GENERAL"}, headers=headers
    )
    ...
    # create comment
    c = client.post(
        "/comments", json={"post_id": post_id, "content": "Nice post"}, headers=headers
    )
    ...
    # get comment
    g = client.get(f"/comments/{comment_id}")
    assert g.status_code == 200
    assert g.json()["content"] == "Nice post"

    # list comments
    lst = client.get(f"/comments?post_id={post_id}")
    assert lst.status_code == 200
    assert any(x["comment_id"] == comment_id for x in lst.json())

    # unlike when not liked => should be idempotent and return 200
    pre_ul = client.post(f"/comments/{comment_id}/unlike", headers=headers)
    assert pre_ul.status_code == 200
    gc_pre = client.get(f"/comments/{comment_id}")
    assert gc_pre.status_code == 200
    assert gc_pre.json()["like_count"] == 0

    # like then unlike
    lk = client.post(f"/comments/{comment_id}/like", headers=headers)
    assert lk.status_code == 200
    gc = client.get(f"/comments/{comment_id}")
    assert gc.status_code == 200
    assert gc.json()["like_count"] == 1

    ul = client.post(f"/comments/{comment_id}/unlike", headers=headers)
    assert ul.status_code == 200
    gc2 = client.get(f"/comments/{comment_id}")
