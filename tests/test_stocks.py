...
def test_stock_and_discussions(client: TestClient):
    ...
    # list stocks
    lst = client.get("/stocks")
    assert lst.status_code == 200
    assert any(x["stock_id"] == stock_id for x in lst.json())

    # get stock
    g = client.get(f"/stocks/{stock_id}")
    assert g.status_code == 200
    assert g.json()["symbol"] == "TST"

    # create discussion
    d = client.post(f"/stocks/{stock_id}/discussions", json={"content": "Hello ticker"}, headers=headers)
    assert d.status_code == 201
    disc = d.json()
    disc_id = disc["discussion_id"]

    # list discussions
    ld = client.get(f"/stocks/{stock_id}/discussions")
